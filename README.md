...
| **Actions** | **A1:** Collect, integrate and clean historical order, customer, product, seller and return data while separating pre-purchase predictors from post-return information. **A2:** Train several classification models and evaluate them using precision, recall, F1-score and other appropriate measures before selecting the recommended solution. |
| **Assumptions** | **AS1:** Historical customer, product, seller and order characteristics contain sufficient patterns to support prediction of future returns. **AS2:** The required datasets can be reliably linked using common identifiers and contain sufficiently complete and accurate records. |
| **Issues** | **I1:** Missing or inconsistent data across STADIOalot's different historical data sources may affect model reliability. **I2:** Return behaviour may vary considerably by product category, seller and customer, which could make generalised predictions less reliable for particular groups. |
| **Decisions** | **D1:** The project will investigate predictive modelling of order returns because STADIOalot's briefing identifies increasing returns as a significant business problem. **D2:** Model selection will prioritise the ability to identify returned orders rather than relying on accuracy alone. |
